...
function exp_mode(base, exponent, n) {
  ...
  var r = bin_array.length;
  var base_array = [];
  var pre_base = base;
  base_array.push(pre_base);
  for (var i = 0; i < r; i++) {
    var next_base = bigint_mod(bigint_mul(pre_base, pre_base), n);
...
function gen_key(p, q) {
  var n = bigint_mul(p, q)                   // 2、令n = p * q。取 φ(n) = (p-1) * (q-1)。
  var fy = bigint_mul(sub_plus(p, t1), sub_plus(q, t1))      // 计算与n互质的整数个数 欧拉函数
  var e = new BigInt("65537");                    // 3、取 e ∈ [1 < e < φ(n) ] ，( n , e )作为公钥对，正式环境中取65537。
  // generate d
  var a = e
  var b = fy
  var g = ext_gcd(a, b)
  var r = g[0]                        // 4、计算ed与fy的模反元素d。令 ed mod φ(n)  = 1，计算d，( n , d ) 作为私钥对。
  var x = g[1]
  var y = g[2]
...
// 解密 c是密文，解密为明文m
// 明文 = 密文 ^ d mod n
function decrypt(c, selfkey) {
  var re = []
  var n = selfkey[0]
  var d = selfkey[1]
  for (var i = 0; i < c.length; i++) {
    var t = new BigInt(c[i])
    var m = exp_mode(t, d, n)
    re.push(m.toString())
  }
...
function ToAssic(s, len) {
  var re = [];
  var temp = "";
  for (var i = 0; i < s.length; i++) {
    if (temp.length >= len-5) {
      re.push(temp);
      temp = "";
    }
    var str = s.charAt(i);
